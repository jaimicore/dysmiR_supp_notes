...
\pagebreak
```{r Aggregated_networks_1, cache=TRUE, include=TRUE, echo=FALSE, eval=TRUE, fig.cap="Network of predicted genes in A) BRCA, B) HNSC, C) LIHC, D) LUAD, E) LUSC, and F) STAD cohorts, genes that could not be connected are not shown. The node colors indicate the number of samples where the gene was predicted. TFs genes are highlighted with orange background.", fig.height = 30, fig.width = 25, fig.align = "center"}
...
<!--   agg.nets.fig <- -->
<!--   plot_grid(nb.subgraphs.per.cohort.gg, -->
<!--             net.list[["UCEC"]], -->
<!--             nrow = 1, labels = c('A)', 'B)'), -->
<!--             align = "hv", -->
<!--             label_size = 35, vjust = 1.25, hjust = 0, rel_widths = c(1, 1)) -->
...
\pagebreak
\begin{landscape}
```{r Breast_Cancer_agg_net, cache=TRUE, include=TRUE, echo=FALSE, eval=TRUE, fig.cap="Network representing the predicted protein-coding genes in BASIS (all samples), BASIS ER+, ER- and BRCA-US cohorts. The name of the genes predicted in two or more cohorts are displayed many times with different colors.", out.width = "600px", out.height="430px", fig.align = "center"}
...
```{r Nb_TFBS_per_gene_PCG, cache=TRUE, include=TRUE, echo=FALSE, eval=TRUE, fig.cap="Distribution of associated TFBSs among predicted, non-predicted, TFs and cancer-related protein-coding genes represented as A) Cumulative Distribution Function, and B) Histogram of frequencies.", fig.height = 10, fig.width = 8, fig.align = "center"}
...
TFBS.xseq.analyzed.genes <- all.associated.features.tab %>% 
...
Nb.TFBSs.per.gene <- TFBS.xseq.analyzed.genes %>% 
                        distinct() %>% 
                        group_by(Associated_gene) %>% 
                        summarise(Nb_TFBS = n(), .groups = "drop") %>% 
                        arrange(desc(Nb_TFBS))

Nb.TFBSs.per.gene$Predicted     <- ifelse(Nb.TFBSs.per.gene$Associated_gene %in% predicted.genes.pancancer, yes = "Predicted", no = "")

Nb.TFBSs.per.gene$Non_Predicted <- ifelse(!Nb.TFBSs.per.gene$Associated_gene %in% predicted.genes.pancancer, yes = "Non-Predicted", no = "")
Nb.TFBSs.per.gene$Cancer <- ifelse(Nb.TFBSs.per.gene$Associated_gene %in% reported.cancer.genes, yes = "Cancer gene", no = "")
Nb.TFBSs.per.gene$TF <- ifelse(Nb.TFBSs.per.gene$Associated_gene %in% TF.genes, yes = "TF", no = "")


Nb.TFBSs.per.gene.classes <- reshape2::melt(Nb.TFBSs.per.gene,
...
## Plot 1: ECDF
TFBS.distr.ecdf.plot <- ggplot(Nb.TFBSs.per.gene.classes, aes(x = Nb_TFBS, colour = Class)) +
...
TFBS.distr.hist.real.plot <- ggplot(Nb.TFBSs.per.gene.classes, aes(x = Nb_TFBS, fill = Class)) +
...
TFBS.distr.hist.log2.plot <- ggplot(Nb.TFBSs.per.gene.classes, aes(x = log2(Nb_TFBS), fill = Class)) +
...
TFBS.distr.hist.log10.plot <- ggplot(Nb.TFBSs.per.gene.classes, aes(x = log10(Nb_TFBS), fill = Class)) +
        geom_histogram(binwidth = 0.1, position = "identity") +
        scale_fill_manual(values = c("#E69F00", "#56B4E9", "#1b9e77", "#d95f02")) +
        theme_bw() +
        facet_grid(Class ~ .)
...
# predicted.TFBS.values <- subset(Nb.TFBSs.per.gene, Predicted_gene == "Predicted")$Nb_TFBS
# non.predicted.TFBS.values <- subset(Nb.TFBSs.per.gene, Predicted_gene == "Non-predicted")$Nb_TFBS
...
# TFBS.distr.ecdf.plot
# TFBS.distr.hist.real.plot
# TFBS.distr.hist.log2.plot
# TFBS.distr.hist.log10.plot
...
```{r Nb_connections_per_PCG, cache=TRUE, include=TRUE, echo=FALSE, eval=TRUE, fig.cap="A", fig.height = 10, fig.width = 8, fig.align = "center"}

# predicted.genes.pancancer
# xseq.analyzed.genes
# reported.cancer.genes
# TF.genes
...
xseq.net.ppi.analyzed.genes <- xseq.net.ppi %>% 
                                filter(Gene %in% xseq.analyzed.genes & Weight >= 0.8) %>% 
                                group_by(Gene) %>% 
                                summarise(Nb_target = n(), .groups = "drop") %>% 
                                arrange(desc(Nb_target))


xseq.net.ppi.analyzed.genes$Predicted     <- ifelse(xseq.net.ppi.analyzed.genes$Gene %in% predicted.genes.pancancer, yes = "Predicted", no = "")
xseq.net.ppi.analyzed.genes$Non_Predicted <- ifelse(!xseq.net.ppi.analyzed.genes$Gene %in% predicted.genes.pancancer, yes = "Non-Predicted", no = "")
xseq.net.ppi.analyzed.genes$Cancer        <- ifelse(xseq.net.ppi.analyzed.genes$Gene %in% reported.cancer.genes, yes = "Cancer gene", no = "")
xseq.net.ppi.analyzed.genes$TF            <- ifelse(xseq.net.ppi.analyzed.genes$Gene %in% TF.genes, yes = "TF", no = "")



xseq.net.ppi.analyzed.genes.classes <- reshape2::melt(xseq.net.ppi.analyzed.genes,
...
## Plot 1: ECDF
TFBS.distr.ecdf.plot <- ggplot(xseq.net.ppi.analyzed.genes.classes, aes(x = Nb_target, colour = Class)) +
        stat_ecdf(size = 1.15, pad = FALSE) +
        theme_bw() +
        scale_color_manual(values = c("#E69F00", "#56B4E9", "#1b9e77", "#d95f02")) + 
...
## Plot 2: histogram with real values
TFBS.distr.hist.real.plot <- ggplot(xseq.net.ppi.analyzed.genes.classes, aes(x = Nb_target, fill = Class)) +
...
```{r Nb_TFBS_per_gene_miRNA, cache=TRUE, include=TRUE, echo=FALSE, eval=TRUE, fig.cap="Distribution of associated TFBSs among predicted, non-predicted, TFs and cancer-related miRNA genes represented as A) Cumulative Distribution Function, and B) Histogram of frequencies.", fig.height = 10, fig.width = 8, fig.align = "center"}
...
predicted.genes.pancancer <- unique(as.vector(unlist(selected.miRNA.list))) %>% gsub(pattern = "::.+$", replacement = "")
xseq.analyzed.genes       <- unique(as.vector(unlist(miRNA.in.cohort.list))) %>% gsub(pattern = "::.+$", replacement = "")


reported.cancer.miRNA <- xseq.sup.mat.tab$mirna_cancer_genes$miRNA
miRNA.genes           <- xseq.sup.mat.tab$mirna_history_mirbase$miRNA
...
Nb.TFBSs.per.miRNA <- TFBS.xseq.analyzed.miRNA %>% 
                        distinct() %>% 
                        group_by(Associated_gene) %>% 
                        summarise(Nb_TFBS = n(), .groups = "drop") %>% 
                        arrange(desc(Nb_TFBS))
...
```{r Corr, cache=TRUE, include=TRUE, echo=FALSE, eval=TRUE, fig.cap="A", fig.height = 10, fig.width = 8, fig.align = "center"}

# xseq.net.ppi.analyzed.genes.classes <- xseq.net.ppi.analyzed.genes.classes %>%
#                                         rename(Value = Nb_target) %>%
#                                         mutate(Type = "Nb_Targets")
# 
# Nb.TFBSs.per.gene.classes <- Nb.TFBSs.per.gene.classes %>%
#                               rename(Gene  = Associated_gene,
#                                      Value = Nb_TFBS) %>%
#                               select(Gene, Value, Class) %>%
#                               mutate(Type = "Nb_TFBS")
# 
# 
# xseq.net.ppi.analyzed.genes.classes %>% head()
# Nb.TFBSs.per.gene.classes %>% head()
# 
# aa <- rbind(xseq.net.ppi.analyzed.genes.classes, Nb.TFBSs.per.gene.classes)
# aa %>% arrange(desc(Gene)) %>% head(10)
