...
## The following variables have the analyzed and predicted gene in each cohort 
## genes.in.cohort.list
## selected.genes.list
aa <- sapply(genes.in.cohort.list, data.frame)
names(aa) <- names(genes.in.cohort.list)
bb <- aa[11:20] ## 1-10 entries are NULL
names(bb)
...
for (cn in names(bb)) {
  ...
  colnames(bb[[cn]]) <- cn
  bb[[cn]]$Predicted <-   ifelse(bb[[cn]][[1]] %in% selected.genes.list[[cn]], yes = "Predicted", no = "Non-Predicted")
  
}

## Generate a Scatter plot + Marginal with each cohort 
