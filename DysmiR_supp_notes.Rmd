...
\pagebreak
```{r Ratio_mutated_vs_selected, cache=TRUE, include=TRUE, echo=FALSE, eval=TRUE, fig.cap="Ratio of dysregulated vs mutated samples. Each panel represents a cohort with all the evaluated genes ranked by the DAC probability. The color of each gene corresponds to the ratio between the number of samples with SSD >= 0.5 (i.e., with high association of dysregulation) divided by the number of mutated samples. The black line corresponds to the cohort-specific threshold defined by FDR of 0.05.", fig.height = 32, fig.width = 25, fig.align = "center"}


xseq.th <- list('BRCA-US'   = 0.5,
                'HNSC-US'   = 0.9999984,
                'LIHC-US'   = 0.9990923,
                'LUAD-US'   = 0.9999987,
                'LUSC-US'   = 0.9999835,
...
for (co in cohorts.all) {
  ...
  if (!grepl(co, pattern = "BASIS")) {
    ...
    load(paste0("xseq_results/xseq_trans_premiRNA_all_mutation_types_all_effects_", co, "_mirna_network_targetScanClean.Rdata")) ## xseq.pred.all.concat
    
  } else {
    
    load(paste0("xseq_results/", co, "_miRNA.Rdata"))
  ...
  xseq.pred.mirna <- xseq.pred.all.concat %>% 
                    ...
                    mutate(Ratio = N_selected_samples/N_mutated_samples,
                           Lab   = ifelse(prob_gene >= xseq.th[[co]], yes = gene, no = "")) %>% 
...
rank.plot <- ggplot(xseq.pred.mirna, aes(y = prob_gene, x = Rank, colour = Ratio, size = N_mutated_samples, label = gene)) +
                ...
                geom_hline(yintercept = xseq.th[[co]]) +
                geom_point(shape = 20) +
                theme_bw() +
                ylim(c(0, 1)) +
